...
export default function UserManagement() {
  ...
  const toggleUserStatus = (id, currentStatus) => {
    ...
    showToast(`ইউজার ${newStatus === 'active' ? 'অ্যাক্টিভ' : 'ডিজেবল'} করা হয়েছে!`);
  };

  const confirmDelete = () => {
    setUsers(users.filter(u => u.id !== deleteModal.userId));
    setDeleteModal({ show: false, userId: null });
    showToast('ইউজার সফলভাবে ডিলিট হয়েছে!', 'error');
  };
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fe] flex flex-col lg:flex-row font-medium relative">
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-center text-slate-500 text-sm mb-6">এই অ্যাকশনটি বাতিল করা যাবে না।</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl relative">
            ...
            <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
              {[
                { label: 'প্যাকেজ', val: detailsModal.user.package },
                { label: 'মোট আপটাইম', val: detailsModal.user.uptime },
                { label: 'ডাটা ইউজ', val: detailsModal.user.dataUsage },
                { label: 'মেয়াদ শেষ', val: detailsModal.user.expireDate, red: true },
              ].map(({ label, val, red }) => (
                <div key={label} className="flex justify-between items-center py-2 border-b border-slate-200/60 last:border-0">
                  <span className="text-slate-500 text-sm">{label}</span>
                  <span className={`font-bold ${red ? 'text-red-500' : 'text-slate-800'}`}>{val}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* সাইডবার */}
      <aside className="w-full lg:w-72 bg-gradient-to-br from-indigo-900 via-blue-900 to-indigo-950 text-white p-6 lg:min-h-screen relative overflow-hidden hidden lg:flex flex-col shadow-2xl">
        <div className="relative z-10 flex-1">
          ...
          <nav className="space-y-3">
            <a href="/" className="flex items-center gap-3 py-3 px-4 hover:bg-white/5 rounded-xl transition-all text-indigo-100 hover:text-white">ড্যাশবোর্ড</a>
            <div className="flex items-center gap-3 py-3 px-4 bg-white/10 backdrop-blur-md rounded-xl border border-white/10 shadow-inner">ইউজার লিস্ট</div>
            <a href="/reports" className="flex items-center gap-3 py-3 px-4 hover:bg-white/5 rounded-xl transition-all text-indigo-100 hover:text-white">সেলস রিপোর্ট</a>
            <a href="/routers" className="flex items-center gap-3 py-3 px-4 hover:bg-white/5 rounded-xl transition-all text-indigo-100 hover:text-white">রাউটার সেটিং</a>
          </nav>
        </div>
      </aside>
      ...
      <main className="flex-1 p-4 md:p-6 lg:p-10 w-full max-w-7xl mx-auto overflow-hidden">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-6 flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-48 relative">
            <select
              ...
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/20 outline-none appearance-none text-sm font-bold text-slate-700"
            >
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-slate-100 shadow-sm">
            ...
            <h3 className="text-lg font-bold text-slate-700">কোনো ইউজার পাওয়া যায়নি</h3>
            <p className="text-slate-500 text-sm mt-1">আপনার সার্চ বা ফিল্টারের সাথে মিল নেই।</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                ...
                <div className="flex justify-between items-start mb-4 pl-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className={`text-[10px] uppercase tracking-wider font-extrabold px-2 py-0.5 rounded-md ${user.status === 'active' ? 'bg-blue-100 text-blue-700' : user.status === 'expired' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>
                        {user.status}
                      </span>
                    </div>
                  </div>
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => toggleUserStatus(user.id, user.status)} className={`p-2 rounded-lg transition-colors ${user.status === 'disabled' ? 'text-emerald-500 hover:bg-emerald-50' : 'text-slate-400 hover:text-orange-500 hover:bg-orange-50'}`}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18.364 5.636a9 9 0 010 12.728m0 0l-2.829-2.829m2.829 2.829L21 21M15.536 8.464a5 5 0 010 7.072m0 0l-2.829-2.829m-4.243 2.829a4.978 4.978 0 01-1.414-2.83m-1.414 5.658a9 9 0 01-2.167-9.238m7.824 2.167a1 1 0 111.414 1.414m-1.414-1.414L3 3m8.293 8.293l1.414 1.414" />
                      </svg>
                    </button>
                    <button onClick={() => setDeleteModal({ show: true, userId: user.id })} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="grid grid-cols-2 gap-y-3 pl-2 text-sm">
                  ...
                  <div className="col-span-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100 flex justify-between items-center mt-1">
                    ...
                    <div className="text-right">
                      <p className="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-0.5">মেয়াদ</p>
