...
const initialLogs = [
  { id: 1, time: '১০:৪৫ AM', router: 'PK-NET Main Router', message: 'সিস্টেম অটো-ব্যাকআপ সম্পন্ন হয়েছে', type: 'info' },
...
// BUG FIX #5: emptyForm দিয়ে সব field সবসময় initialize করা হচ্ছে
const emptyForm = { name: '', ip: '', port: '8728', apiUser: 'admin', apiPass: '' };

export default function RouterManagement() {
  const [routers, setRouters] = useState([]);
  // BUG FIX #6b: logs কে state হিসেবে রাখা হয়েছে — আগে initialLogs সরাসরি JSX-এ
  // ব্যবহার হত, তাই "ক্লিয়ার লগস" কাজ করত না এবং logs.length এ ReferenceError আসত।
  const [logs, setLogs] = useState(initialLogs);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [toast, setToast] = useState({ show: false, message: '', type: '' });
  const [deleteModal, setDeleteModal] = useState({ show: false, routerId: null });
  // BUG FIX #5: সবসময় তিনটি field সহ routerModal initialize
  const [routerModal, setRouterModal] = useState({ show: false, editMode: false, data: null });
  const [routerForm, setRouterForm] = useState(emptyForm);
  ...
  const openAddModal = () => {
    setRouterForm(emptyForm);
    setRouterModal({ show: true, editMode: false, data: null });
  };

  const openEditModal = (router) => {
    setRouterForm({ name: router.name, ip: router.ip, port: router.port, apiUser: 'admin', apiPass: '' });
    setRouterModal({ show: true, editMode: true, data: router });
  };

  // BUG FIX #5: closeModal সবসময় তিনটি field সহ reset করে।
  // আগে শুধু setRouterModal({ show: false }) ছিল — editMode ও data undefined থেকে যেত।
  const closeModal = () => {
    setRouterModal({ show: false, editMode: false, data: null });
  };
  ...
  // BUG FIX #6: form submit এ এখন routerForm state থেকে ডেটা পড়া হয়।
  // আগে submit করলে modal বন্ধ হত কিন্তু কোনো ডেটা সেভ হত না।
  const handleSaveRouter = (e) => {
    e.preventDefault();
    if (routerModal.editMode && routerModal.data) {
      setRouters(routers.map(r =>
        r.id === routerModal.data.id
          ? { ...r, name: routerForm.name, ip: routerForm.ip, port: routerForm.port }
          : r
      ));
      showToast('রাউটার আপডেট করা হয়েছে!');
    } else {
      const newRouter = {
        id: Date.now(),
      ...
      showToast('নতুন রাউটার যুক্ত করা হয়েছে!');
    }
    closeModal();
  };
  ...
  const confirmDelete = () => {
    ...
    showToast('রাউটার সফলভাবে রিমুভ করা হয়েছে!', 'error');
  };
      ...
      {routerModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] p-6 md:p-8 max-w-lg w-full shadow-2xl">
            ...
            {/* BUG FIX #6: controlled inputs — routerForm state-এর সাথে যুক্ত */}
            <form onSubmit={handleSaveRouter} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 mt-1 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/20 outline-none text-slate-700"
                  placeholder="e.g. PK-NET Main"
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text" required
                    value={routerForm.ip}
                    onChange={(e) => setRouterForm({ ...routerForm, ip: e.target.value })}
                    className="w-full px-4 py-3 mt-1 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/20 outline-none text-slate-700"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setRouterForm({ ...routerForm, apiUser: e.target.value })}
                    className="w-full px-4 py-3 mt-1 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/20 outline-none text-slate-700"
                    placeholder="admin"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="password"
                    value={routerForm.apiPass}
                    onChange={(e) => setRouterForm({ ...routerForm, apiPass: e.target.value })}
                    className="w-full px-4 py-3 mt-1 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-500/20 outline-none text-slate-700"
                    placeholder="••••••••"
                  />
                </div>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className={`flex-1 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-colors flex justify-center items-center gap-2 ${testingConnection ? 'cursor-wait' : ''}`}
                >
                  {testingConnection
                    ? <span className="animate-spin w-5 h-5 border-2 border-slate-500 border-t-transparent rounded-full"></span>
                    : 'কানেকশন টেস্ট'}
                </button>
                <button type="submit" className="flex-1 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-500/30 transition-all">
                  সেভ করুন
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteModal({ show: false, routerId: null })} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-colors">বাতিল</button>
      ...
      <main className="flex-1 p-4 md:p-8 w-full max-w-[1600px] mx-auto overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-800 tracking-tight flex items-center gap-3">
              রাউটার ম্যানেজমেন্ট
              {refreshing && (
                <span className="flex items-center gap-1 text-xs bg-indigo-50 text-indigo-600 px-2 py-1 rounded-full">
                  <span className="w-1.5 h-1.5 bg-indigo-600 rounded-full animate-ping"></span> সিঙ্কিং...
                </span>
              )}
            </h1>
            <p className="text-slate-500 mt-1">মাইক্রোটিক রাউটার যুক্ত করুন এবং লাইভ স্ট্যাটাস মনিটর করুন</p>
          </div>
          <button onClick={openAddModal} className="bg-indigo-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/30 flex items-center justify-center gap-2">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
          {[
            ...
            { title: 'গড় CPU ইউজ', val: '১১%', icon: 'M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z', color: 'text-orange-500', bg: 'bg-orange-50' },
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
          {loading ? (
            ...
            routers.map(router => (
              <div key={router.id} className="bg-white/80 backdrop-blur-md rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden relative group hover:shadow-xl transition-all">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1 font-semibold flex items-center gap-2">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-md uppercase tracking-wider ${router.status === 'online' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>{router.ping}</span>
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-100">
                      <button onClick={() => openEditModal(router)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-white rounded-lg transition-all">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                      </button>
                      <button onClick={() => setDeleteModal({ show: true, routerId: router.id })} className="p-2 text-slate-400 hover:text-red-600 hover:bg-white rounded-lg transition-all">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100/50">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden max-w-[40px]">
                          <div className={`h-full ${router.cpu > 80 ? 'bg-red-500' : 'bg-blue-500'}`} style={{ width: `${router.cpu}%` }}></div>
                        </div>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden max-w-[40px]">
                          <div className="h-full bg-indigo-500" style={{ width: `${router.ram}%` }}></div>
                        </div>
                      </div>
                    </div>
                    <div>
        ...
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden mb-8">
          <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            {/* BUG FIX #6b: setLogs([]) এখন সত্যিই কাজ করে কারণ logs এখন state */}
            <button onClick={() => setLogs([])} className="text-sm font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors">
              ক্লিয়ার লগস
            </button>
          </div>
          <div className="divide-y divide-slate-100">
            {logs.length === 0 ? (
              <div className="p-8 text-center text-slate-500">কোনো লগ পাওয়া যায়নি।</div>
            ) : (
              logs.map(log => (
